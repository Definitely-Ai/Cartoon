...
import {approvedGeneratedCartoons} from './cartoon-review-server';
...
export async function galleryCollection(){return [...await approvedGeneratedCartoons(),...cartoonCollection];}
export async function visibleGallery(){const [collection,ids]=await Promise.all([galleryCollection(),hiddenCartoonIds()]);const hidden=new Set(ids);return collection.filter(c=>!hidden.has(c.id));}
export async function setCartoonHidden(id:unknown,hidden:unknown){
  if(typeof id!=='string'||typeof hidden!=='boolean'||!(await galleryCollection()).some(c=>c.id===id))throw new AutomationError(400,'Choose a cartoon in this collection.');
