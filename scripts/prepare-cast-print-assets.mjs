...
for(const id of ['drew','barclay','abby']){
  const input=await fs.readFile(id==='barclay'?identity.portraitPath:path.join(source,id+'-print-portrait.png'));
...
await fs.writeFile(path.join(output,'manifest.json'),JSON.stringify({edition:'September 2026',portraits:report,barclayIdentity:identity.id,method:'Current presentation portraits. Barclay uses the exact owner-approved September 14 reference portrait, matched by the versioned cartoon acting plates. Strict grayscale delivery.'},null,2)+'\n');
