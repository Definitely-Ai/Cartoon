...
// An editorial sequence, not population rank. End the city section on connection.
const ORDER=['nyc-whole-tour','la-new-excuse','houston-reading-fee','dallas-budget-dinner','boston-boxes-missed-meeting','sf-reason-to-stay','miami-rest-of-building','phoenix-smart-tree','dc-every-penny','chicago-bigger-plans','atlanta-one-destination','seattle-bench-with-view'];
export default function PresentationShow({cartoons,canManage}:{cartoons:BestOfCartoon[];canManage:boolean}){
  const cities=ORDER.map(id=>cartoons.find(c=>c.id===id)).filter((c):c is BestOfCartoon=>!!c);
  const slides:Slide[]=[{kind:'opening',title:'A familiar place to see ourselves'},{kind:'system',title:'The system behind the scene'},{kind:'demo',title:'Make the next local edition'},{kind:'places',title:'Twelve cities. Twelve ways in.'},...cities.map(cartoon=>({kind:'cartoon' as const,title:cartoon.cityLabel||cartoon.title,cartoon})),{kind:'closing',title:'A reason to return'}];
  ...
  const cover=cities.find(c=>c.id==='seattle-bench-with-view')||cartoons[0];
  ...
  return <>
    <section className="presentation-launch" aria-labelledby="show-launch-title">
      <div><p className="showcase-kicker">September 15, 2026 · The national city edition</p><h2 id="show-launch-title">Made for the big screen.</h2><p>{cities.length} new city cartoons, a clear system walkthrough, and a live generation demonstration—all on this page.</p></div>
    ...
    <div ref={stage} className={`presentation-show ${open?'presentation-show-open':''}`} hidden={!open} tabIndex={-1} role="dialog" aria-modal={open?true:undefined} aria-label="The Swinging Door large-screen presentation">
      {open&&<>
        ...
        <div className={`presentation-content presentation-${slide.kind}`} ref={content}>
          ...
          {slide.kind==='places'&&<><p className="presentation-eyebrow">One edition · September 15, 2026</p><h2>Twelve cities. Twelve ways in.</h2><p className="presentation-lead">A home. A commute. A family. Something that feels like your life.</p><div className="presentation-city-grid">{cities.map(c=><button key={c.id} onClick={()=>move(slides.findIndex(s=>s.cartoon?.id===c.id))}><strong>{c.cityLabel}</strong><span>{c.theme}</span></button>)}</div><p className="presentation-subtle">Prepared, editor-curated examples with fresh local TV art. Current source context is documented; not every topic is breaking news today.</p></>}
          {slide.kind==='cartoon'&&slide.cartoon&&<div className="presentation-split">
            <div className="presentation-copy"><p className="presentation-eyebrow">{slide.cartoon.cityLabel}</p><h2 className="presentation-theme">{slide.cartoon.theme}</h2><blockquote className="presentation-quote">“{slide.cartoon.caption}”</blockquote><p className="presentation-speaker">— {slide.cartoon.speaker}</p><div className="presentation-art-tools"><button onClick={()=>setNotes(n=>!n)} aria-expanded={notes}>{notes?'Hide context':'Source & local context'}</button><a href={slide.cartoon.src} target="_blank" rel="noreferrer">Open full-resolution art</a></div>{notes&&<aside className="presentation-source"><p>{slide.cartoon.context}</p><a href={slide.cartoon.sourceUrl} target="_blank" rel="noreferrer">{slide.cartoon.sourceTitle}</a><p>Checked September 15, 2026. Dialogue and illustration are fictional.</p></aside>}</div>
