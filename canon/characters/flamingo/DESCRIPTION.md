# The Flamingo — Canonical Description

<!-- BRAND: replace when final — character name pending -->

Purpose: the single authoritative physical description of the flamingo, written so the exact same block can be pasted into every image prompt and every artist brief without edits.

## Canonical Description Block

> Write the exact prose block used verbatim in every image prompt: species, build, head and face features, eyes, attire, distinguishing marks, and line weight. One paragraph, no synonyms, no alternatives — the block is copied, never paraphrased.
