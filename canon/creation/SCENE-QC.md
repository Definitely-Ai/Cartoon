# Scene QC — look before you file

<!-- BRAND: replace when final — "The Swinging Door" is the working series/bar name throughout this document -->

Purpose: the last gate between a generated image and the founder's table. This is a **visual inspection of the actual pixels**, run by the drawing AI on every candidate, after generation and before `file_cartoon`. Reading the prompt back is not inspection — look at the image.

**The rule: any failed line is a redraw, never a filing.** Redraw with the fault named explicitly in the prompt ("Abby must be BEHIND the bar; the counter hides her from the upper hip down"), then inspect again. If a candidate fails twice on the same line, change the composition rather than rerolling.

Every failure below has actually happened. This list grows — when the founder's feedback names a new recurring fault, add it here.
...
- [ ] **Hands and grips.** Contact points are real: fingers (or Drew's feather-digits) actually wrap what they hold, with the right digit count. No mitten blobs, no pass-through grips.
- [ ] **Occlusion is correct.** Behind the bar, the counter hides Abby from the upper hip down — that is right, not a fault. Never draw her lower body through or in front of the counter to show it.
- [ ] **Scale holds.** Character sizes stay consistent with the counter, the stools, and each other, panel to panel.
...
- [ ] Strictly black-and-white: paper white, one mid-gray wash, solid black — no color anywhere, including the flag pin.
- [ ] No speech balloons, no thought bubbles, no caption text in the art. Lettering only on the setting's named carriers — in the bar: the reversed window sign, the chalkboard, and the TV; outdoors: the news prop — all short.
- [ ] One clear focal action; generous white space; walls not crowded.
- [ ] The gag reads in two seconds without the caption.

## Away games

Outdoors, section 1 applies with the furniture translated: seats are benches, thwarts, or folding chairs; surfaces are coolers, tables, or the ground. Section 3's lettering rule translates too: no window sign, chalkboard, or TV exists outdoors — the only lettering allowed is the news prop's two or three short words. Water, ground, and horizon stay level. Everything else is identical.
