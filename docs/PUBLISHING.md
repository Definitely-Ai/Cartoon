...
3. **Add the finished cartoon** as `cartoon.png` inside the folder. Create a black-and-white square illustration at least 1200px wide, finalize the `caption` in `meta.json`, then run `cd site && npm run dialogue`. That appends the exact words in a magazine-style field below the art. The finished portrait PNG and the JSON caption must match; square art without its dialogue is blocked by the build.
...
4. **Fill in `meta.json`.** The schema, one line per field:
   ...
   ```json
   {
     ...
     "caption": "…",                    // required non-empty string — exact words printed in the PNG
...
The build validates every cartoon and **fails loudly rather than shipping bad data**. Every error message names the offending folder. Common errors are:
...
2. In the file-name box, type the new folder name followed by a slash — e.g. `cartoons/2026-08-13-the-quarterly-call/` — then drag in a finished portrait `cartoon.png` that already contains the exact dialogue below the illustration. GitHub creates the folder.
